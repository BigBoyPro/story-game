...
import {Lobby, LobbySettings, LogLevel, OpError, SocketEvent, Story, StoryElement} from "../../../shared/sharedTypes.ts";
...
export const onLobbySettings = (callback: (lobbySettings: LobbySettings) => void) => {
    socket.on(SocketEvent.LOBBY_SETTINGS, (lobbySettings) => {
        callback(lobbySettings);
    });
}

export const offLobbySettings = () => {
    socket.off(SocketEvent.LOBBY_SETTINGS);
}
...
export const submitLobbySettings = (lobbyCode: string, lobbySettings: LobbySettings) => {
    request(SocketEvent.SUBMIT_LOBBY_SETTINGS, userId, lobbyCode, lobbySettings);
}
